...
### Docs
- **Refreshed `README.md` + `PICKER.md` to cover the features added since the screenshot/text were last updated (the v1.7.x era).** The README Driver Picker section now documents the per-driver gear/throttle/RPM input cluster, the positions-gained/lost-vs-grid arrows (▲/▼), the retired-car treatment (maroon `RETIRED` pill + 42% row fade), the parked/broken-car frozen-telemetry suppression, and the built-in Replay panel; `PICKER.md` gains a *Positions gained / lost vs grid* section, a *Retired / out-of-race treatment* section, the position-arrow column entry, and version-history rows for v1.8.0 (Replay panel) and v1.10.15–v1.10.18. The `picker-overview.png` hero screenshot was refreshed to the 2026 Barcelona GP (Hamilton's first win as a Ferrari driver, shot on v1.10.18) and the picker intro was reframed around that win; the picker TOC anchor was fixed.
