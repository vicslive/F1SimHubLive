...
| [docs/CLOCKS.md](docs/CLOCKS.md) | **Read before touching the session clock/countdown.** The one formula both the picker header and wheel dashboard share (`SessionEndUtc − (playhead − PlaybackLead)`), the three MV time signals and which to trust, the four traps that caused 1.10.7–1.10.12 regressions (Newtonsoft `Z`-drop +5h, driver-reset playhead, `-:--:--` placeholder, hour-format), the DO-NOT-BREAK invariants, a file/line map, and the full regression history. |
